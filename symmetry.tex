...
The term that has been added, $\exp⁡[-2πi(\vec{Q}\cdot\vec{U_j})]$, defines an \textbf{atomic displacement parameter} for atom \textit{j}, as a tensor $\vec{U_j}$, noting that the subscript \textit{j} allows different values or even levels of expansion for each atom.
