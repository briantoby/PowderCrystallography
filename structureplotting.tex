...
GSAS-II provides structural visualization as part of the Phase data tree entry. This chapter introduces some of the capabilities offered there. 
While this can be used for publication-quality graphics and there may well be things that GSAS-II's molecular visualization does that would be hard to duplicate in other software, it should also be noted that there are many other crystallographic graphics packages that may be better for many tasks. These include the free Mercury\index{software!Mercury} and Vesta\index{software!Vesta} programs, as well as CrystalMaker\index{software!CrystalMaker} and Diamond\index{software!Diamond}. However, where GSAS-II's graphics are most useful is in its integration into the structure completion and refinement process, which will be the focus of what I present here. Before going into the GUI for structure visualization, it is worthwhile to first describe what is actually displayed. 
...
The default atom color is set from the value for the element type shown on the General tab. The default can be changed there. When any of the methods given in \S\ref{ref:atomchangeselection} is used, a dialog will be presented for color selection, but the nature of this dialog will be determined by the operating system on your computer. 
...
When a Fourier map is computed, an extra section appears in the data window, as shown in  in Fig. \ref{fig:DrawOptsF}. These options affect how Fourier maps and peaks from that map are displayed and are discussed in \S\ref{ref:FourierViewOpts}.
