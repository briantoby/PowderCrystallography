...
\begin{itemize}
    ...
    \item Create void map: this sweeps through the unit cell on the specified grid spacing and locates points where there are no atoms (as determined by their space-filling radius) within a specified radius. Blue dots are then placed at those locations where there are no atoms. These dots can be quite hard to see against a black background, so you may want to change the background color from the ``Draw Options'' tab. GSAS-II really needs a command that allows you to change the void map color. 
