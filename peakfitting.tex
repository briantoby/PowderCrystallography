...
One possibility is that the symmetry of the material has been lowered. GSAS-II provides a fairly simple way to look for that using the visualization in the Unit Cells List data tree entry.  This is discussed in \S\ref{ref:supercellindexing}.
...
\begin{itemize}
    \item Copy the limits. This is done by selecting the Limits data tree entry for the fitted histogram and then use the ``Edit Limits''/Copy menu command to copy limits from the selected histogram to the others.
    \item Copy the background: This is done by selecting the Background data tree entry for the fitted histogram and then use the ``Background''/Copy menu command to copy the background settings from the selected histogram to the others.
    \item Copy the peaks: For this select the ``Peak List'' data tree entry for the fitted histogram. Before you copy settings, you will likely want to turn off some of the refinement flags. For example, perhaps start with refining peak intensities only and refine more parameters later. Use the ``Peak Fitting''/``Peak copy'' menu command to copy the entire peak table from the selected histogram to all others. 
\end{itemize}
...
\begin{itemize}
    \item Copy to next: When this is selected, the refinement results from one histogram are copied to the next as the refinement starting point. This is very useful for parametric work where one seting of starting points would not be good for all histograms, but not very much is changing between histograms that are adjacent in the list.
    \item Reverse order: Normally histograms are process in a top-to-bottom order with respect to the histogram list, though only the histograms selected in the window are used for peak fitting. When this option is selected, the order is bottom-to-top. 
\end{itemize}
