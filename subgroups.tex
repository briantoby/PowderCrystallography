...
Further, there are also tools provided for working with distortion mode generated via representational analysis (see \S\ref{ref:RA}). 
...
In the menu labeled ``Cell Index/Refine'' the ``Cell Symmetry Search'' command runs the NIST*LATTICE program\index{software!NIST*LATTICE} from Vicky Karen\index{Karen, V.} and the late Alan Mighell\index{Mighell, A.}.
...
In the menu labeled ``Cell Index/Refine'' the ``Run Subgroups'' command runs the  SUBGROUPS program\index{software!SUBGROUPS} from the Bilbao Crystallographic Server (BCS).\index{Bilbao Crystallographic Server}\index{software!BCS}\index{software!Bilbao (Crystallographic Server)} 
...
In the menu labeled ``Cell Index/Refine'' the ``Run k-SUBGROUPSMAG'' command runs the k-SUBGROUPSMAG program\index{software!k-SUBGROUPSMAG}  from the Bilbao Crystallographic Server (BCS).
...
GSAS-II is will handle non-standard space group settings, and these settings can be very useful (as discussed in \S\ref{ref:nonstandardSG}), but it is also useful to have the unit cell and coordinates in the standard setting. 
The ``Std Setting'' menu command in the ``Compute'' menu runs the ``IDENTIFY GROUP'' program\index{software!IDENTIFY GROUP}  from the Bilbao Crystallographic Server. 
...
The ``SUBGROUPS search'' menu command in the ``Compute'' menu runs the ``SUBGROUPS'' program\index{software!SUBGROUPS} from the Bilbao Crystallographic Server. This duplicates the ``Cell Index/Refine''/``Run Subgroups'' command from the histogram ``Index Cells List'' data tree entry. 
...
The ``ISOCIF Supergroup search''  command from the Compute menu command for the General tab of the phase runs the ``ISOCIF' program\index{software!ISOCIF} from the Brigham Young University's ISOTROPY Software Suite web site\index{software! ISOTROPY Software Suite}. This will search for a possible higher symmetry versions of the current phase. It has succeeded in cases where the Bilbao PSEUDO search has not, but provides much less information on what is being done. I believe there is value in trying both. 

\subsection{Phase: Isodistort tab}
The ISODISTORT tab associated with each phase provides a mechanism for running the  ``ISODISTORT'' program\index{software!ISODISTORT} from the Brigham Young University's ISOTROPY web site. This can be used in two different ways. In the first, a single structure is uploaded to the website and then displacive distortion modes for that structure are generated as single irrep modes. The structure can be read from a CIF or can be the current phase. Select the input and then use the Operations/``Run ISODISTORT'' to submit that information to the web server. A table of subgroups is created and CIFs for any of those child structures can be written.  
...
Now, you can generate these graphs as needed with BCS program SUBGROUPGRAPH\index{software!SUBGROUPGRAPH} (in the Group-Subgroup Relations grouping). 
...
\section{The BYU ISOTROPY Software Suite Web Server}\label{ref:isotropy}
The Brigham Young University ISOTROPY Software Suite server\index{software!ISOTROPY Software Suite}\index{ISOTROPY Software Suite}, at web address \url{https://iso.byu.edu/}, is also an international treasure. It provides a collection of $\approx 20$ utility programs, which are largely motivated by representational analysis. Some of these offer very sophisticated analyses and graphical tools. The backbone of the site is the ISOTROPY program\index{software!ISOTROPY program}, which, to the best of my knowledge, is not in general distribution, but can be run online from the web site. Note that each utility entry has an extensive help page with documentation and, at least where I have looked, an annotated example. One of the really neat tools built into this website is a visualization program called IsoVIZ\index{software!IsoVIZ} where one can vary the distortion modes generated by the programs and see how the structure is changed by the mode or combinations of modes. 

As noted before, if a CIF is generated on this web site that contains displacive, occupancy or magnetic distortion modes, when the CIF is read by GSAS-II, the distortion modes are converted to constraints that can then be used in refinements. 
...
\begin{quote}
This is the canonical reference for space groups. If your library does not have a physical copy of Volume A, they may offer on-line access.
\end{quote}

\textit{International Tables for Crystallography, Volume A1: Symmetry Relations Between Space Groups.} H. Wondratschek, U. Müller eds. (2004). Dordrecht: Springer, for the International Union of Crystallography. DOI:  10.1107/97809553602060000101.
\begin{quote}
This greatly expands on the subgroup-supergroup information found in \textit{International Tables, Volume A}, but the Bilbao Crystallographic Server (see below) is probably even easier to use. 
\end{quote}

\item
The Bilbao Crystallographic Server, \url{https://cryst.ehu.es/cryst/} offers excellent tools for exploring subgroup-supergroup relationships. 
...
\begin{quote}
This explains how to read the \textit{International Tables, Volume A}. Excellent discussion on subgroup-supergroup relationships. 
\end{quote}
...
\begin{quote}
This also explains how to read the \textit{International Tables, Volume A}. 
\end{quote}

\item
\textit{Symmetry Relationships between Crystal Structures}, (2017) U. Müller, Oxford Publ. for the International Union of Crystallography. DOI: 10.1002/anie.201306902.
\end{itemize}
